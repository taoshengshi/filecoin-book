...
1，你认为当前区块链有哪些误区？
A：基建不完善，不接地气。可能监管不足也是一个问题。

2，发展截止到2019年，区块链新的共识和技术的发展有哪些创新和突破？
layer 1 在提高TPS方面的进展一个是Bitcoin _NG，一个是DAG分片，但在工程上还没有真正受到检验。共识算法方面，基于有用共识和POS的结合，像Filecoin，是不错的进展。layer2的 链下提高TPS的一些技术非常实用，在一段时间内会解决一些问题，伴随基建设施的完善，这些技术也会持续发挥作用。发展会是这样：bitcoin->ethereum->filecoin。filecoin融合了layer1和layer2的新技术进展。

3，区块链在开源领域的现状？
原创不多，都是基于已有的几家开源软件修修改改。但不像Linux这样的开源软件，bitcoin和ethereum不能很快和产业结合。

4，区块链在金融领域的应用现状？
这一块我不擅长，长期看好。金融服务实体，还要看区块链如何和实体产业结合。

https://bloxroute.com 的创新也不是太多，类似互联网时代的CDN。我认为他们的背景和思路不错，创始人在Akamai做过CDN，现在用类似的思路解决区块链的问题，这个产品的主要客户将会是矿池和大的公链。


可验证网络 是一个方向。作为基础设施，适合公链，私链，联盟链，也能和CDN，IOT对接类比一下：
1，bitcoin和ethereum矿池 基于区块链的泡沫 抢的是已有云计算的蛋糕

2，Filecoin矿池 基于filecoin市场网络 抢的是已有云存储的蛋糕

3，可验证网络  基于网络，服务公链和用户，抢的是CDN的蛋糕 但可以扩展到计算和存储，抢的是已有基础设施，包括云计算和云存储的蛋糕，可验证 是核心属性。